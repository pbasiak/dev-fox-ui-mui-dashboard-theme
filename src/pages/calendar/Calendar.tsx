  ...
  const tileContent = ({ date, view }: { date: Date, view: string }) => {
    ...
    if (view === 'month') {
      const events = data.find((data) => {
        ...
        return date.getFullYear() === newDate.getFullYear() && date.getMonth() === newDate.getMonth() && date.getDate() === newDate.getDate();
      });

      if (events?.events) {
        const eventsList = events.events.sort((a, b) => {
          return new Date(a.startDate).getTime() - new Date(a.endDate).getTime() > new Date(b.startDate).getTime() - new Date(b.endDate).getTime() ? 1 : -1;
        }).map((event) => {
          ...
          return (
            <Box className={'react-calendar__tile-content'} data-id={event.id} key={event.id} sx={{
              position: 'relative',
              mb: 0.5,
              ...
              <Typography fontSize={12} textAlign={'left'} title={event.name} textOverflow={'ellipsis'} overflow={'hidden'} whiteSpace={'nowrap'}>
                {!isMultiDayEvent && (<Typography component={'span'} fontSize={'inherit'}>{eventTime} </Typography>)}
